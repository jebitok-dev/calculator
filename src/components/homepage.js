import React from 'react';

const Home = () => (
  <div className="centered">
    <h1>Welcome to our page!</h1>
    <p>
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Libero vel
      quibusdam veritatis quaerat excepturi exercitationem numquam eligendi ut,
      mollitia voluptate!
    </p>
    <p>
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Corrupti mollitia
      ullam voluptatem quam exercitationem!
    </p>
    <p>
      Lorem ipsum dolor, sit amet consectetur adipisicing elit. Recusandae nihil
      praesentium reiciendis ut fugiat. Voluptas rem natus aut magni assumenda.
    </p>
  </div>
);

export default Home;
